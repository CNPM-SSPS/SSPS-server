...
export const updateSupportTicketByOfficer = async (req, res) => {
    ...
    try {
        ...
        .findByIdAndUpdate(supportTicketID, { response, status : 'closed', closedAt: new Date() , officer : officer }, { new: true });
...
export const getSupportTicketsByStudent = async (req, res) => {
    const student = req.user._id;
    try {
        const supportTickets = await SupportTicket.find({ student }).populate('student').populate('printer').populate('officer');
...
export const getSupportTicketsByOfficer = async (req, res) => {
    ...
    try {
        const supportTickets = await SupportTicket.find({ 
            ...
            printingErrorID: errorID }).populate('student').populate('printer').populate('officer');
        res.status(200).json(supportTickets);
    }
    catch (error) {
        res.status(400).json({ message: error.message });
...
export const viewSupportTicket = async (req, res) => {
    ...
    try {
        ...
        .findById(supportTicketID).populate('student').populate('printer').populate('officer');
