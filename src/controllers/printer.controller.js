...
export const updatePrinter = async (req, res) => {
    ...
    if(Object.keys(printer).length === 0) return res.status(400).send('No data to update, please provide data');
    if (!mongoose.Types.ObjectId.isValid(_id)) return res.status(404).send('No printer with that id');
    const lastModified = new Date();
    const updatedPrinter = await Printer.findByIdAndUpdate(_id, {...printer,lastModified}, {new: true});
    if(!updatePrinter){
